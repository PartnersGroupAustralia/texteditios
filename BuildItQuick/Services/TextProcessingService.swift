...
nonisolated enum TextProcessingService: Sendable {
    ...
    static func apply(_ step: ProcessingStep, to text: String) -> String {
        ...
        case .fixPasswordsInColumn: fixPasswordsInColumn(text, column: step.columnIndex, delimiter: step.delimiter)
        }
    }
    ...
    static func fixPasswordsInColumn(_ text: String, column: Int, delimiter: String) -> String {
        guard !delimiter.isEmpty else { return text }
        return text.components(separatedBy: "\n")
            .map { line in
